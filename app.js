...
function routeManeuverPath(kind) {
  const paths = {
    'keep-left': '<path d="M40 68V43c0-10-4-16-14-22L15 14"/><path d="M17 31l-2-17 17 3"/>',
    'keep-right': '<path d="M24 68V43c0-10 4-16 14-22l11-7"/><path d="M47 31l2-17-17 3"/>',
    'slight-left': '<path d="M43 68V48c0-10-4-17-13-23L17 15"/><path d="M19 33l-2-18 18 3"/>',
    'slight-right': '<path d="M21 68V48c0-10 4-17 13-23l13-10"/><path d="M45 33l2-18-18 3"/>',
    'half-left': '<path d="M45 68V47c0-11-5-17-15-22L15 17"/><path d="M19 35l-4-18 18 1"/>',
    'half-right': '<path d="M19 68V47c0-11 5-17 15-22l15-8"/><path d="M45 35l4-18-18 1"/>',
    left: '<path d="M48 68V43c0-9-6-15-15-15H13"/><path d="M27 14L13 28l14 14"/>',
    right: '<path d="M16 68V43c0-9 6-15 15-15h20"/><path d="M37 14l14 14-14 14"/>',
    'sharp-left': '<path d="M50 68V42c0-11-7-18-18-18H13"/><path d="M28 10L13 24l15 15"/>',
    'sharp-right': '<path d="M14 68V42c0-11 7-18 18-18h19"/><path d="M36 10l15 14-15 15"/>',
    uturn: '<path d="M47 68V34c0-13-7-21-19-21S9 21 9 34v11"/><path d="M-1 34L9 45l11-11"/>',
    roundabout: '<path d="M32 68V51"/><circle cx="32" cy="33" r="17"/><path d="M45 21l10-6"/><path d="M48 10l7 5-1 9"/>'
  };
  return paths[kind] || '<path d="M32 68V13"/><path d="M18 28L32 13l14 15"/>';
}
...
function updateVisibleRouteTurnMarkers() {
  ...
  if (!routeTurnMarkerLayer || !routeTurnMarkerSpecs.length) return;
  const visibleBounds = map.getBounds().pad(0.12);
  routeTurnMarkerLayer.clearLayers();

  for (const spec of routeTurnMarkerSpecs) {
    if (!visibleBounds.contains(spec.latlng)) continue;
    const icon = L.divIcon({
      className: 'route-turn-marker',
      html: `<div class="route-maneuver-arrow" style="transform:rotate(${spec.heading}deg)"><svg viewBox="0 0 64 76" aria-hidden="true">${routeManeuverPath(spec.kind)}</svg></div>`,
      iconSize: [58, 76],
      iconAnchor: [29, 60]
    });
  ...
  if (coordinates.length > 1) {
    ...
    routeTurnMarkerSpecs = maneuvers
      ...
      .map(instruction => {
        ...
        const center = pointAtProgress(coordinates, progress.cumulative, markerProgress).point;
        const before = pointAtProgress(coordinates, progress.cumulative, Math.max(0, markerProgress - 8)).point;
        const after = pointAtProgress(coordinates, progress.cumulative, Math.min(progress.total, markerProgress + 8)).point;
        return {
          latlng: L.latLng(center.lat, center.lon),
          heading: bearing(before, after),
