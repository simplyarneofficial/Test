...
function sampleRoutePoints(coordinates, maxSamples = 48) {
...
async function fetchRoundaboutData(coordinates) {
  const samples = sampleRoutePoints(coordinates, 42);
  const aroundQueries = samples.map(([lon, lat]) => `way(around:90,${lat.toFixed(6)},${lon.toFixed(6)})["junction"~"roundabout|circular"]`).join(';');
  const query = `[out:json][timeout:24];(${aroundQueries};)->.r;node(w.r)->.rn;way(bn.rn)->.connected;(.r;.connected;);out body geom;`;
  ...
  const ways = (data.elements || []).filter(element => element.type === 'way' && Array.isArray(element.geometry));
  const roundaboutWays = ways.filter(way => /roundabout|circular/.test(way.tags?.junction || ''));
  const connectedWays = ways.filter(way => !/roundabout|circular/.test(way.tags?.junction || ''));
  return { roundaboutWays, connectedWays };
}

function nearestCoordinateIndex(coordinates, point, start = 0, end = coordinates.length - 1) {
  let best = { index: start, distance: Infinity };
  for (let i = Math.max(0, start); i <= Math.min(end, coordinates.length - 1); i += 1) {
    const d = distanceMeters({ lat: coordinates[i][1], lon: coordinates[i][0] }, point);
    if (d < best.distance) best = { index: i, distance: d };
  }
  return best;
}

function roundaboutRouteSpan(coordinates, way) {
  ...
  for (let i = 0; i < coordinates.length; i += 1) {
    ...
    let min = Infinity;
    for (const node of geometry) min = Math.min(min, distanceMeters(routePoint, node));
    if (min <= 16) hits.push(i);
  }
...
function countRoundaboutExit(way, connectedWays, routeSpan, coordinates) {
  const nodes = way.geometry.map(point => ({ lat: point.lat, lon: point.lon }));
  if (nodes.length < 4) return 1;
  const entryPoint = { lat: coordinates[routeSpan.start][1], lon: coordinates[routeSpan.start][0] };
  const exitPoint = { lat: coordinates[routeSpan.end][1], lon: coordinates[routeSpan.end][0] };
  const entryNode = nearestCoordinateIndex(nodes.map(p => [p.lon, p.lat]), entryPoint);
  const exitNode = nearestCoordinateIndex(nodes.map(p => [p.lon, p.lat]), exitPoint);
  const routeMidIndex = Math.floor((routeSpan.start + routeSpan.end) / 2);
  const routeMid = { lat: coordinates[routeMidIndex][1], lon: coordinates[routeMidIndex][0] };

  function walk(direction) {
    const indices = [];
    let i = entryNode.index;
    for (let guard = 0; guard < nodes.length + 2; guard += 1) {
      i = (i + direction + nodes.length) % nodes.length;
      indices.push(i);
      if (i === exitNode.index) break;
    }
    const midpoint = nodes[indices[Math.floor(indices.length / 2)] || entryNode.index];
    return { indices, fit: distanceMeters(midpoint, routeMid) };
  }
  const forward = walk(1);
  const backward = walk(-1);
  const traversed = forward.fit <= backward.fit ? forward.indices : backward.indices;

  const roundNodeKeys = new Set(nodes.map(nodeKey));
  const exitNodes = new Set();
  for (const index of traversed) {
    const node = nodes[index];
    for (const road of connectedWays) {
      if (!road.tags?.highway || ['service','track','path','footway','cycleway'].includes(road.tags.highway)) continue;
      const touches = road.geometry.some(point => distanceMeters(node, { lat: point.lat, lon: point.lon }) < 4);
      if (!touches) continue;
      const outside = road.geometry.some(point => !roundNodeKeys.has(nodeKey({ lat: point.lat, lon: point.lon })) && distanceMeters(node, { lat: point.lat, lon: point.lon }) > 8);
      if (outside) exitNodes.add(index);
    }
  }
  return Math.max(1, exitNodes.size);
}

async function enrichInstructionsWithRoundabouts(coordinates, progressData, instructions) {
  const { roundaboutWays, connectedWays } = await fetchRoundaboutData(coordinates);
  if (!roundaboutWays.length) return instructions;
  const roundabouts = [];
  for (const way of roundaboutWays) {
    const span = roundaboutRouteSpan(coordinates, way);
    if (!span) continue;
    const exitNumber = countRoundaboutExit(way, connectedWays, span, coordinates);
    const progress = progressData.cumulative[span.start] || 0;
    roundabouts.push({ span, progress, exitNumber, way });
  }
  if (!roundabouts.length) return instructions;

  let result = instructions.filter(instruction => !roundabouts.some(roundabout => {
    const startProgress = progressData.cumulative[roundabout.span.start] || 0;
    const endProgress = progressData.cumulative[roundabout.span.end] || startProgress;
    return instruction.progress >= startProgress - 20 && instruction.progress <= endProgress + 20 && instruction.kind !== 'arrive';
  }));

  for (const roundabout of roundabouts) {
    const point = coordinates[roundabout.span.start];
    result.push({
      index: roundabout.span.start,
      point,
