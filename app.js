...
$('startNavigationBtn').addEventListener('click', () => {
  ...
  watchId = navigator.geolocation.watchPosition(async position => {
    ...
    if (navigationFollowing) {
      map.setView([snappedPosition.lat, snappedPosition.lon], 18, { animate: true });
      if (window.matchMedia('(orientation: landscape) and (max-height: 600px)').matches) {
        map.panBy([Math.min(190, window.innerWidth * 0.18), 0], { animate: true });
      }
    }
...
$('recenterBtn').addEventListener('click', () => {
  navigationFollowing = true;
  if (snappedPosition) map.setView([snappedPosition.lat, snappedPosition.lon], 18, { animate: true });
});
...
function refreshMapLayout() {
  window.setTimeout(() => {
    ...
    if (watchId !== null && navigationFollowing && snappedPosition) {
      map.setView([snappedPosition.lat, snappedPosition.lon], 18, { animate: false });
      if (window.matchMedia('(orientation: landscape) and (max-height: 600px)').matches) {
