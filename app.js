...
function spokenManeuverText(instruction) {
  ...
  if (instruction.kind === 'left') return 'links abbiegen';
  if (instruction.kind === 'right') return 'rechts abbiegen';
  if (instruction.kind === 'arrive') return 'Du hast dein Ziel erreicht';
  return instruction.text || 'geradeaus weiterfahren';
}
...
function announceInstruction(instruction, instructionIndex, distance) {
  ...
  const stages = [
    { threshold: 500, key: '500', text: d => `In ${Math.round(d / 50) * 50} Metern ${spokenManeuverText(instruction)}.` },
    { threshold: 220, key: '200', text: d => `In ${Math.max(100, Math.round(d / 50) * 50)} Metern ${spokenManeuverText(instruction)}.` },
    { threshold: 75, key: 'now', text: () => `Jetzt ${spokenManeuverText(instruction)}.` }
  ];
