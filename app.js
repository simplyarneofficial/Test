...
function drawRoute(feature) {
  ...
  if (coordinates.length > 1) {
    const progress = buildRouteProgress(coordinates);
    const spacing = Math.max(55, Math.min(105, progress.total / 28));
    for (let distance = spacing; distance < progress.total - 20; distance += spacing) {
      const center = pointAtProgress(coordinates, progress.cumulative, distance).point;
      const before = pointAtProgress(coordinates, progress.cumulative, Math.max(0, distance - 7)).point;
      const after = pointAtProgress(coordinates, progress.cumulative, Math.min(progress.total, distance + 9)).point;
      const heading = bearing(before, after);
      const icon = L.divIcon({
        className: 'route-direction-marker',
        html: `<div class="route-direction-arrow" style="transform:rotate(${heading}deg)"></div>`,
        iconSize: [24, 24], iconAnchor: [12, 12]
      });
      L.marker([center.lat, center.lon], { icon, interactive:false, keyboard:false, zIndexOffset:520 }).addTo(group);
    }

    const maneuvers = currentRoute?.instructions || buildInstructions(coordinates);
    maneuvers.forEach(instruction => {
      if (!instruction || instruction.kind === 'straight' || instruction.kind === 'arrive' || instruction.progress < 12) return;
      const at = pointAtProgress(coordinates, progress.cumulative, instruction.progress).point;
      const icon = L.divIcon({
        className: 'route-turn-marker',
        html: `<div class="route-turn-arrow ${instructionClass(instruction)}"></div>`,
        iconSize:[34,34], iconAnchor:[17,17]
      });
      L.marker([at.lat, at.lon], { icon, interactive:false, keyboard:false, zIndexOffset:650 }).addTo(group);
    });
  }
