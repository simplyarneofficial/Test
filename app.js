...
function updateManeuverIcon(instruction) {
  ...
  const paths = {
    ...
    roundabout: `<path d="M32 56V47"/><path d="M32 47a16 16 0 1 1 13-7"/><path d="M45 40V25h15"/><path d="M51 17l9 8-9 8"/>${instruction?.exitNumber ? `<text x="32" y="37" text-anchor="middle" class="roundabout-exit-number">${instruction.exitNumber}</text>` : ''}`,
...
function buildRouteArrowGeometry(spec) {
  // Der Pfeil übernimmt den projizierten Verlauf der Route und wird an der
  // Mitte dieses Verlaufs verankert. Dadurch liegt er auch in Kurven und
  // Kreisverkehren exakt auf der blauen Linie, statt daneben zu schweben.
  const BOX_W = 76;
  const BOX_H = 94;
  const PAD = 9;
  const beforeMeters = spec.kind === 'roundabout' ? 25 : 29;
  const afterMeters = spec.kind === 'roundabout' ? 14 : 18;
  const samples = 24;
  const projected = [];
  ...
  for (let i = 0; i <= samples; i += 1) {
    ...
    projected.push(map.latLngToLayerPoint([geo.lat, geo.lon]));
  }

  const minPX = Math.min(...projected.map(point => point.x));
  const maxPX = Math.max(...projected.map(point => point.x));
  const minPY = Math.min(...projected.map(point => point.y));
  const maxPY = Math.max(...projected.map(point => point.y));
  const pixelCenter = L.point((minPX + maxPX) / 2, (minPY + maxPY) / 2);
  const markerLatLng = map.layerPointToLatLng(pixelCenter);

  const local = projected.map(point => ({
    x: point.x - pixelCenter.x,
    y: point.y - pixelCenter.y
  }));
  ...
  const scale = Math.min((BOX_W - PAD * 2) / width, (BOX_H - PAD * 2) / height, 2.4);
  const fitted = local.map(point => ({
    x: BOX_W / 2 + point.x * scale,
    y: BOX_H / 2 + point.y * scale
  }));

  // Glatte SVG-Kurve statt kantiger Einzelstücke.
  let path = `M${fitted[0].x.toFixed(1)} ${fitted[0].y.toFixed(1)}`;
  for (let i = 1; i < fitted.length - 1; i += 1) {
  ...
  const finalPoint = fitted[fitted.length - 1];
  path += ` T${finalPoint.x.toFixed(1)} ${finalPoint.y.toFixed(1)}`;

  const tip = finalPoint;
  const previous = fitted[fitted.length - 4] || fitted[fitted.length - 2];
  const angle = Math.atan2(tip.y - previous.y, tip.x - previous.x);
  const headLength = 12;
  const headSpread = 0.62;
  const left = {
  ...
  return {
    latlng: markerLatLng,
    html: `<svg viewBox="0 0 ${BOX_W} ${BOX_H}" aria-hidden="true"><path class="route-arrow-outline" d="${path}"/><path class="route-arrow-line" d="${path}"/><path class="route-arrow-head-outline" d="${head}"/><path class="route-arrow-head" d="${head}"/></svg>`
  };
}
...
function updateVisibleRouteTurnMarkers() {
  ...
  const geometry = buildRouteArrowGeometry(spec);
  const icon = L.divIcon({
    className: 'route-turn-marker',
    html: `<div class="route-maneuver-arrow">${geometry.html}</div>`,
    iconSize: [76, 94],
    iconAnchor: [38, 47]
  });
  L.marker(geometry.latlng, {
    icon,
    interactive: false,
    keyboard: false,
    zIndexOffset: 720
