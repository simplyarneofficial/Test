...
function drawNextArrow(step){
  clearRouteArrow();
  const a=anchor(step);if(!a||!map.getBounds().pad(.08).contains([a.lat,a.lng]))return;
  const geometry=step.geometry?.coordinates||[];if(geometry.length<2)return;
  const centerIndex=Math.max(1,Math.min(geometry.length-2,Math.floor(geometry.length*.55)));
  const from=Math.max(0,centerIndex-4),to=Math.min(geometry.length,centerIndex+5);
  const points=geometry.slice(from,to).map(([lng,lat])=>[lat,lng]);if(points.length<2)return;
  const end=points.at(-1),prev=points.at(-2);if(!end||!prev)return;
  const angle=bearing({lat:prev[0],lng:prev[1]},{lat:end[0],lng:end[1]});
  const halo=L.polyline(points,{pane:'arrowPane',className:'route-arrow',color:'#073d75',weight:14,opacity:.96,lineCap:'round',lineJoin:'round'});
  const front=L.polyline(points,{pane:'arrowPane',className:'route-arrow',color:'#fff',weight:7,opacity:1,lineCap:'round',lineJoin:'round'});
  const icon=L.divIcon({className:'',iconSize:[42,42],iconAnchor:[21,21],html:`<div style="font-size:38px;color:white;transform:rotate(${angle}deg);filter:drop-shadow(0 3px 5px #000);line-height:42px;text-align:center">↑</div>`});
  const marker=L.marker(end,{pane:'arrowPane',icon,interactive:false,zIndexOffset:900});
  state.routeArrowGroup=L.layerGroup([halo,front,marker]).addTo(map);
}
